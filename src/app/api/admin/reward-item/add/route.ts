...
export async function POST(req: NextRequest) {
  try {
    ...
    if (dbImagePath) {
      sql = `
        INSERT INTO ms_rewards (category_id, reward_name, vendor_name, image_path, points_required, stock)
        VALUES (?, ?, ?, ?, ?, ?);
      `;
      params = [category_id, reward_name, vendor_name, dbImagePath, points_required, stock];
    } else {
      sql = `
        INSERT INTO ms_rewards (category_id, reward_name, vendor_name, points_required, stock)
        VALUES (?, ?, ?, ?, ?);
      `;
      params = [category_id, reward_name, vendor_name, points_required, stock];
    }
