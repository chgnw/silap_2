...
export async function POST(req: NextRequest) {
  try {
    ...
    const existing_icon = formData.get("existing_icon") as string;
    ...
    const checkSql = `
      ...
      WHERE tier_name = ? AND id != ?
    `;
    const existing = (await query(checkSql, [tier_name, id])) as any[];
    ...
    // Handle file upload
    let tierIconUrl: string | null = existing_icon || null;
    if (file && file.size > 0) {
      const allowedTypes = [
        "image/jpeg",
        "image/jpg",
    ...
    return NextResponse.json(
      { error: "Failed to update tier", detail: error.message },
      { status: 500 }
    );
  }
}
