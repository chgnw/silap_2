...
export default function RewardsManagementPage() {
  ...
  const [deleteTarget, setDeleteTarget] = useState<{ type: 'category' | 'reward'; id: number } | null>(null);
  ...
  */
  const handleSaveReward = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.message === "SUCCESS") {
        showToast("success", `Reward ${rewardMode === 'add' ? 'added' : 'updated'} successfully!`);
  ...
  */
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      if (result.message === "SUCCESS") {
        showToast("success", `${deleteTarget.type === 'category' ? 'Category' : 'Reward'} berhasil di hapus!`);
        setIsDeleteModalOpen(false);
        deleteTarget.type === 'category' ? fetchCategories() : fetchRewards();
      } else {
        showToast("failed", `Gagal menghapus ${deleteTarget.type === 'category' ? 'Category' : 'Reward'}!`);
      }
    } catch (error) {
  ...
  const columnsReward: ColumnDef<Reward>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <img 
          src={row.original.image_path ? `/upload/${row.original.image_path}` : '/images/dummy.png'} 
    ...
    },
    {
      ...
      header: 'Points',
      cell: ({ row }) => row.original.points_required.toLocaleString(),
    },
    {
      accessorKey: 'stock',
      header: 'Stock',
  ...
  return (
    <div className={styles.pageContainer}>
      <div className={styles.pageHeader}>
        ...
        <i className={styles.pageSubtitle}>Manage reward categories and redemption items with points and stock.</i>
      </div>
      ...
      <Modal
        ...
        title={`${catMode === 'add' ? 'Add' : catMode === 'edit' ? 'Edit' : 'Detail'} Reward Category`}
      >
      ...
      <Modal
        ...
        title={`${rewardMode === 'add' ? 'Add' : rewardMode === 'edit' ? 'Edit' : 'Detail'} Reward`}
      >
          ...
          <div className={styles.formSection}>
            ...
            <div className={styles.formGroup}>
              ...
              <input 
                ...
                onChange={(e) => setRewardForm({...rewardForm, vendor_name: e.target.value})}
                disabled={rewardMode === 'view'}
            ...
            <div className={styles.formGroup}>
              ...
              <select 
                ...
                onChange={(e) => setRewardForm({...rewardForm, category_id: e.target.value})}
                disabled={rewardMode === 'view'}
                required
              >
            ...
            <div className={styles.modalFooter}>
              {rewardMode === 'view' ? (
                <button type="button" onClick={() => setIsRewardModalOpen(false)} className={`${styles.btnBase} ${styles.btnCancel}`}>Close</button>
              ) : (
                <>
                  <button type="button" onClick={() => setIsRewardModalOpen(false)} className={`${styles.btnBase} ${styles.btnCancel}`}>Cancel</button>
                  <button type="submit" className={`${styles.btnBase} ${styles.btnSave}`} disabled={isSubmitting}>
                    {isSubmitting ? 'Saving...' : 'Save'}
                  </button>
                </>
